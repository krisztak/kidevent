...
async function upsertUser(
  ...
  // First try to find user by Replit ID
  let existingUser = await storage.getUserByReplitId(claims["sub"]);
  
  if (!existingUser && claims["email"]) {
    // If not found by Replit ID, try by email
    existingUser = await storage.getUserByEmail(claims["email"]);
  }

  if (existingUser) {
    // Update existing user with Replit data if needed
...
export const createIsAdminMiddleware = (storage: any): RequestHandler => {
